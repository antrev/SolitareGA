function shuffleDeck(a) {
    ...
    var a = [
        ...
        { id: '5s', suite: 'spades', color: 'black', value: 5, name: "5 of spades", image: "playing_cards/png-cards-1.3/5_of_spades.png" },
...
function drawPiles(piles) {
    ...
    piles.forEach(function(pile, index) {
        $('.piles').append('<div class="pile" pile_id=' + index + ' />');
...
function validateMove(deckCard, pileCard) {
    if (deckCard.color !== pileCard.color && (deckCard.value === pileCard.value - 1)) {
        return true
    } else {
        return false
    }
...
function deckMove(pileCard, deckCard) {
    if (pileCard.color !== deckCard.color) {
        alert('valid move');
        return true
    } else {
        alert('invalid move');
...
$(document).ready(function() {
    ...
    $('.pile').on('click', function(e) {
        var $pile = $(e.currentTarget),
            pileId = $pile.attr('pile_id'),
            thisPile = piles[pileId],
            lastCard = thisPile[thisPile.length - 1];

        if (validateMove(deck[deck.length - 1], lastCard)) {
            thisPile.push(deck.pop());
